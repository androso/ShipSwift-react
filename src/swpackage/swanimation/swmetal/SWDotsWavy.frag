...
//
 ...
 float swDotsWavyHeight(float x, float z, float t,
...
vec4 swDotsWavy(vec2 position, vec4 color, vec4 boundingRect, float time, float speed, float brightness, vec4 tint, vec4 background, float dotSize, float gridDensity, float patternScale, float vignette, float horizon, float amplitude, float depthFade) {
    ...
    for (int j = jMin; j <= jMax; j++) {
        ...
        for (int di = -1; di <= 1; di++) {
            ...
            float distPx      = length(position - vec2(dotScreenX, dotScreenY));
...
void main() {
  ...
  fragColor = swDotsWavy(position, vec4(0.0), vec4(0.0, 0.0, uSize.x, uSize.y), uTime, uSpeed, uBrightness, uTint, uBackground, uDotSize, uGridDensity, uPatternScale, uVignette, uHorizon, uAmplitude, uDepthFade);
}
