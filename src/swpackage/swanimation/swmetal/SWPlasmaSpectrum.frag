...
vec4 swPlasmaSpectrum(vec2 position, vec4 color, vec4 boundingRect, float time, vec4 c1, vec4 c2, vec4 c3, vec4 c4, vec4 c5, float scale, float intensity, float distortion) {
    ...
    float v1 = sin(dot(p + vec2(0.0,  0.045), d) * 2.4 + swPlasmaFBM3((p + vec2(0.0,  0.045)) * 1.3 + time * 0.07) * distortion * 4.0 + time * 0.45);
    float v2 = sin(dot(p,                       d) * 2.4 + swPlasmaFBM3( p                       * 1.3 + time * 0.07) * distortion * 4.0 + time * 0.45);
    float v3 = sin(dot(p + vec2(0.0, -0.045), d) * 2.4 + swPlasmaFBM3((p + vec2(0.0, -0.045)) * 1.3 + time * 0.07) * distortion * 4.0 + time * 0.45);

    vec3 cc1 = vec3(c1.rgb), cc2 = vec3(c2.rgb), cc3 = vec3(c3.rgb),
           cc4 = vec3(c4.rgb), cc5 = vec3(c5.rgb);
    vec3 ca = swPlasmaPal5(v1 * 0.5 + 0.5, cc1, cc2, cc3, cc4, cc5);
    vec3 cb = swPlasmaPal5(v2 * 0.5 + 0.5, cc1, cc2, cc3, cc4, cc5);
    vec3 cc = swPlasmaPal5(v3 * 0.5 + 0.5, cc1, cc2, cc3, cc4, cc5);
    vec3 col = vec3(ca.r, cb.g, cc.b) * intensity * 1.15;
    return vec4(vec3(clamp(col, 0.0, 1.6)), 1.0);

}

void main() {
  vec2 position = vec2(gl_FragCoord.x, uSize.y - gl_FragCoord.y);
  fragColor = swPlasmaSpectrum(position, vec4(0.0), vec4(0.0, 0.0, uSize.x, uSize.y), uTime, uC1, uC2, uC3, uC4, uC5, uScale, uIntensity, uDistortion);
}
