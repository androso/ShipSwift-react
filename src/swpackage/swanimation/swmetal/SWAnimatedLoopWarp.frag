...
vec4 swAnimatedLoopWarp(vec2 position, vec4 color, vec4 boundingRect, float time, float speed, float lineWidth, float lines, float spacing, float channelOffset, float patternMod, float rotation, float scale, vec2 center, float shape, float petals, float angularLobes, float angularAmount, float angularSpeed, vec4 color1, vec4 color2, vec4 color3, vec4 background) {
    ...
    float d   = length(uv * vec2(0.4, 1.0));
    float pmm = max(patternMod, 0.0001);
    float m   = mod(uv.x, pmm);
...
void main() {
  ...
  fragColor = swAnimatedLoopWarp(position, vec4(0.0), vec4(0.0, 0.0, uSize.x, uSize.y), uTime, uSpeed, uLineWidth, uLines, uSpacing, uChannelOffset, uPatternMod, uRotation, uScale, uCenter, uShape, uPetals, uAngularLobes, uAngularAmount, uAngularSpeed, uColor1, uColor2, uColor3, uBackground);
}
