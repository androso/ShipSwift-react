...
vec4 swPlasmaPrism(vec2 position, vec4 color, vec4 boundingRect, float time, vec4 c1, vec4 c2, vec4 c3, vec4 c4, vec4 c5, float scale, float intensity, float distortion) {
    ...
    float v1 = sin(dot(p + vec2( 0.025, 0.0), d) * 3.0 + swPlasmaFBM2((p + vec2( 0.025, 0.0)) * 1.5) * distortion * 3.0 + time * 0.4);
    float v2 = sin(dot(p,                       d) * 3.0 + swPlasmaFBM2( p                       * 1.5) * distortion * 3.0 + time * 0.4);
    float v3 = sin(dot(p + vec2(-0.025, 0.0), d) * 3.0 + swPlasmaFBM2((p + vec2(-0.025, 0.0)) * 1.5) * distortion * 3.0 + time * 0.4);
    ...
    vec3 col = vec3(ca.r, cb.g, cc.b) * intensity;
    return vec4(vec3(clamp(col, 0.0, 1.5)), 1.0);

}

void main() {
  vec2 position = vec2(gl_FragCoord.x, uSize.y - gl_FragCoord.y);
  fragColor = swPlasmaPrism(position, vec4(0.0), vec4(0.0, 0.0, uSize.x, uSize.y), uTime, uC1, uC2, uC3, uC4, uC5, uScale, uIntensity, uDistortion);
}
