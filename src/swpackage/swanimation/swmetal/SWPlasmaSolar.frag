...
vec4 swPlasmaSolar(vec2 position, vec4 color, vec4 boundingRect, float time, vec4 c1, vec4 c2, vec4 c3, vec4 c4, vec4 c5, float scale, float intensity, float distortion) {
...
void main() {
  ...
  fragColor = swPlasmaSolar(position, vec4(0.0), vec4(0.0, 0.0, uSize.x, uSize.y), uTime, uC1, uC2, uC3, uC4, uC5, uScale, uIntensity, uDistortion);
}
