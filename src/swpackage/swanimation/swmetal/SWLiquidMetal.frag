...
// =============================================================================
 ...
 float SWLM_PI = 3.14159265358979323846;
 vec4 SWLM_C = vec4(0.211324865405187,
                                 0.366025403784439,
                                -0.577350269189626,
                                 0.024390243902439);
...
// (toward 0) for a darker, higher-contrast body.
 vec3 SWLM_COLOR1     = vec3(0.97, 0.98, 1.0);   // cool highlight white
 vec3 SWLM_COLOR2     = vec3(0.035, 0.035, 0.045);// near-black shadow base
 float  SWLM_COLOR2_BLUE_BOOST = 0.05;  // extra blue added to shadow toward bottom-right (bigger = bluer shadow)

// --- Cool ambient bounce -----------------------------------------------------
// The blue/cyan tint mixed into the mid-tones. More blue / less red+green =
// cooler. Multiplied at runtime by the `coolTint` slider.
 vec3 SWLM_COOL_COLOR = vec3(0.55, 0.72, 1.0);   // ambient cool-blue bounce
...
// rim ring reads before the `fresnel` slider applies (bigger = fatter rim).
 vec3 SWLM_RIM_COLOR  = vec3(0.60, 0.80, 1.0);   // cool-BLUE grazing rim (WWDC blue edge)
 float  SWLM_RIM_GAIN   = 4.0;   // rim ring width gain (bigger = wider rim band)
 float  SWLM_RIM_GLOW   = 0.7;   // additive cool-blue edge glow — keeps the rim bright even on dark frames (bigger = brighter blue edge)
...
// a touch so it never dies to pure black.
 float  SWLM_TOPDOWN_FLOOR = 0.55;  // bottom darkening factor in lowField (smaller = darker bottom)
 float  SWLM_TOPDOWN_LIFT  = 0.05;  // floor lift so bottom never goes pure black (bigger = brighter bottom)
...
// NOISE_WOBBLE = how much noise bends the otherwise-straight light edge.
 vec2 SWLM_FLOW_AXIS   = vec2(0.62, 0.78);  // direction the bright bar sweeps along (rotate to re-aim)

// --- Rectangular sweep bar (THE bright block) — minimal: one bar, back  forth
 float  SWLM_FIELD_SCALE = 1.1;   // scale of the big light/dark blocks — SMALLER = BIGGER blocks (~1 = one big bright block + one big shadow across the logo)
 float  SWLM_FLOW_DRIFT  = 0.6;   // one-way flow speed of the big blocks (bigger = faster continuous drift)
...
// (Legacy organic-field params below are now UNUSED — kept until the look is locked, then cleaned.)
 float  SWLM_RAMP_LO     = -0.15;  // ramp start (more negative = larger bright body)
 float  SWLM_RAMP_HI     = 0.95;   // ramp end   (closer to LO = harder light/dark edge)
 float  SWLM_BRIGHT_LO   = 0.34;   // bright-zone threshold low  (lower = more of logo is lit)
 float  SWLM_BRIGHT_HI   = 0.66;   // bright-zone threshold high (closer to LO = harder edge)
 float  SWLM_CORE_LO     = 0.66;   // specular core threshold low
 float  SWLM_CORE_HI     = 0.95;   // specular core threshold high
 float  SWLM_FIELD_BASE  = 0.30;   // baseline brightness of the field (bigger = brighter overall)
 float  SWLM_FIELD_RAMP_W= 0.58;   // weight of the bright ramp (bigger = stronger body/shadow split)
 float  SWLM_FIELD_N1_W  = 0.16;   // weight of coarse noise blob (bigger = more organic variation)
 float  SWLM_FIELD_N2_W  = 0.08;   // weight of finer noise blob
 float  SWLM_OUT_BASE    = 0.05;   // lowField output floor (bigger = shadows never as deep)
 float  SWLM_OUT_BRIGHT_W= 0.82;   // bright body contribution to output
 float  SWLM_OUT_CORE_W  = 0.28;   // specular core contribution to output
 float  SWLM_FLOW_SPEED  = 0.26;   // diagonal slide speed of the big reflection (bigger = faster)
 float  SWLM_NOISE_WOBBLE= 0.16;   // how much noise bends the light edge (bigger = wavier)
 float  SWLM_FIELD_N1_FREQ = 1.15; // coarse blob frequency (bigger = smaller blobs)
 float  SWLM_FIELD_N2_FREQ = 2.30; // finer blob frequency
...
// Bigger AMP = more contrast in the stripe modulation.
 float  SWLM_BULGE_BASE  = 0.5;    // bulge midpoint
 float  SWLM_BULGE_AMP   = 0.45;   // bulge swing from noise (bigger = stronger stripe modulation)
 float  SWLM_BULGE_FREQ  = 1.3;    // bulge blob frequency
 float  SWLM_BULGE_FLOW  = 0.11;   // bulge blob drift speed

// --- Stripe vs. block blend --------------------------------------------------
// blockMix = clamp_01(1 - FADE * (patternScale - PIVOT)). At/below PIVOT the
...
// it the periodic stripes fade back in. Raise FADE to kill stripes sooner.
 float  SWLM_BLOCK_FADE  = 0.80;   // how fast stripes fade out as scale drops (bigger = block wins sooner)
 float  SWLM_BLOCK_PIVOT = 0.55;   // patternScale at/below which it's pure block field
...
// clip's overall rise  fall (runs on an independent slow clock).
 float  SWLM_BREATH_MIN   = 0.82;  // dimmest point of the breath (smaller = deeper dips)
 float  SWLM_BREATH_SPEED = 1.0;   // breath rate rad/s (~6.3s per cycle at 1.0)

// --- Local warm accent -------------------------------------------------------
// A drifting touch of warm gold in the hottest reflections — cool-silver
// dominant with a hint of warmth, not pure greyscale.
 vec3 SWLM_WARM_COLOR = vec3(1.0, 0.66, 0.42);  // warm orange-gold accent
 float  SWLM_WARM_AMT   = 0.18;   // warmth in hot reflections (0 = pure cool)
...
// visible "block edges" where the smooth field crosses a smoothstep threshold.
 float  SWLM_DITHER     = 2.2;   // dither strength in 1/255 units (bigger = noisier but smoother gradients)

// =============================================================================

 vec3 swLM_mod289v3(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
 vec2 swLM_mod289v2(vec2 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
 vec3 swLM_permute(vec3 x)  { return swLM_mod289v3(((x * 34.0) + 1.0) * x); }

// 2D simplex noise (Ashima Arts / Stefan Gustavson, public domain).
 float swLM_snoise(vec2 v) {
    vec2 i = floor(v + dot(v, SWLM_C.yy));
    vec2 x0 = v - i + dot(i, SWLM_C.xx);
    vec2 i1 = (x0.x > x0.y) ? vec2(1.0, 0.0) : vec2(0.0, 1.0);
    vec4 x12 = x0.xyxy + SWLM_C.xxzz;
    x12.xy -= i1;
    i = swLM_mod289v2(i);
    vec3 p = swLM_permute(swLM_permute(i.y + vec3(0.0, i1.y, 1.0))
    ...
    m = m * m;
    m = m * m;
    vec3 x = 2.0 * fract(p * SWLM_C.www) - 1.0;
    vec3 h = abs(x) - 0.5;
    vec3 ox = floor(x + 0.5);
    vec3 a0 = x - ox;
...
// Soft alpha falloff at the layer's outer 10%.
 float swLM_imgFrameAlpha(vec2 uv, float frameWidth) {
...
// many thin lines.
 float swLM_getColorChannel(float c1, float c2,
                                  float stripePos,
                                  vec3 w,
                                  float extraBlur,
                                  float bulge,
...
// sweep so the big reflections creep diagonally like mercury.
 float swLM_lowField(vec2 uv, float topDown, float flow) {
    ...
    vec2 dir   = normalize(SWLM_FLOW_AXIS);
    vec2 fuv   = uv - dir * (flow * SWLM_FLOW_DRIFT);            // one-way continuous flow, no seam
    float  n1    = swLM_snoise(fuv * SWLM_FIELD_SCALE);
    float  n2    = swLM_snoise(fuv * SWLM_FIELD_SCALE * 1.7 + vec2(4.0, 9.0));
    ...
    return clamp_01(SWLM_OUT_BASE + (1.0 - SWLM_OUT_BASE) * s);
}
...
vec4 swLiquidMetal(vec2 position, int layer, vec4 boundingRect, float time, float speed, float refraction, float edge, float liquid, float patternBlur, float patternScale, float timeScale, float coolTint, float fresnel, float bandSoftness) {
// 1..8 broad-band reflection blur
    vec2 sz = boundingRect.zw;
    vec2 uvRaw = position / max(sz, vec2(1.0));
    vec2 uv = uvRaw;

    vec4 img = layerSample(position);
    if (img.a == 0.0) {
        return img;
    }
    ...
    vec3 color1 = SWLM_COLOR1 * mix(0.85, 1.0, topDown);
    vec3 color2 = vec3(SWLM_COLOR2.r, SWLM_COLOR2.g,
    ...
    vec3 coolColor = SWLM_COOL_COLOR;
    ...
    vec2 gradUV = uv - 0.5;
    float blobLow = swLM_snoise(uv * SWLM_BULGE_FREQ + vec2(SWLM_BULGE_FLOW * t, -0.08 * t));
    ...
    float opacity = 1.0 - smoothstep(0.9 - 0.5 * clamp_01(edge),
                                     1.0 - 0.5 * clamp_01(edge), pixelEdge);
    opacity *= swLM_imgFrameAlpha(uvRaw, 0.1);

    float noise = swLM_snoise(uv - vec2(t, t));
    // liquid edge-noise distortion REMOVED — it animated the silhouette mask as
    // a second moving layer on top of the blob. Edge mask is now  so only
    ...
    refr_r *= clamp_01(refraction);
    refr_b *= clamp_01(refraction);

    vec3 w = vec3(thin1Width, thin2Width, wideRatio);
    ...
    float r = swLM_getColorChannel(color1.r, color2.r, stripe_r, w,
                                   0.02 + 0.03 * clamp_01(refraction) * bulge,
    ...
    vec3 blockColor = mix(color2, color1, lowF);
    ...
    metal = mix(metal, metal * coolColor, clamp_01(coolTint) * midBand);
    ...
    float warmGate = smoothstep(0.6, 0.95, lowF) *
                     (0.5 + 0.5 * swLM_snoise(uv * 0.8 - vec2(0.12 * t, 0.05 * t)));
    metal = mix(metal, metal * SWLM_WARM_COLOR, clamp_01(warmGate) * SWLM_WARM_AMT);
    ...
    float e = clamp_01(pixelEdge);
    float rim = clamp_01(SWLM_RIM_GAIN * e * (1.0 - e)) * clamp_01(fresnel);
    rim *= rim;   // tighten the ring so the rim hugs the very edge
    vec3 rimColor = SWLM_RIM_COLOR;   // cool-blue grazing reflection
    ...
    float dither = (fract(sin(dot(position, vec2(12.9898, 78.233))) * 43758.5453) - 0.5) * (SWLM_DITHER / 255.0);
    metal += dither;

    return vec4(float(metal.r * opacity),
                 float(metal.g * opacity),
                 float(metal.b * opacity),
                 float(opacity));
...
void main() {
  ...
  fragColor = swLiquidMetal(position, 0, vec4(0.0, 0.0, uSize.x, uSize.y), uTime, uSpeed, uRefraction, uEdge, uLiquid, uPatternBlur, uPatternScale, uTimeScale, uCoolTint, uFresnel, uBandSoftness);
}
